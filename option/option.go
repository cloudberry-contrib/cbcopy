...
const (
	...
	ALLOW_PARTITION_RESHAPE = "allow-partition-reshape"
	QUIET                   = "quiet"
...
func (o Option) validatePartTables(title string, tables []*DbTable, userTables map[string]TableStatistics, dbname string) {
	for _, t := range tables {
		...
		_, exists := userTables[k]
		if !exists {
			gplog.Fatal(errors.Errorf("%v \"%v\" does not exist on \"%v\" database. "+
				"If this is a partition leaf or intermediate partition level, specify the root partition table instead.", title, k, dbname), "")
		}
	}
}
...
func (o Option) ValidateDestTables(userTables map[string]TableStatistics, dbname string) {
	for _, t := range o.destTables {
		...
		_, exists := userTables[k]
		if !exists {
			gplog.Fatal(errors.Errorf("dest table \"%v\" does not exist on \"%v\" database", k, dbname), "")
		}
	}
}
